	...
	let further_processing = match arg_type.get(0).cloned().flatten() {
		Some(_) => quote! {
			...
			let cb = move |arg: #ser_type| {
				...
				send_message(<std::sync::MutexGuard<Option<u32>> as std::ops::DerefMut<Target = Option<u32>>>::deref_mut(from.lock().unwrap()).cloned().unwrap(), handler_name.as_ptr() as i32, arg);
			};
		},
		None => quote! {},
	};

...
pub fn with_bindings(args: TokenStream, input: TokenStream) -> TokenStream {
	...
	fn gen_der(
		...
		let mut clone_all =
			arg_types
				.iter()
				.fold(Vec::new(), |mut accum: Vec<TokenStream2>, (i, (pat, _))| {
					let prev = if i == &0usize {
						TokenStream2::new()
					} else {
						accum[i - 1].clone()
					};
	...
	let further_processing = match arg_type.get(0).cloned().flatten() {
		Some(_) => quote! {
			...
			let cb = move |arg: #ser_type| {
				...
				send_message(<std::sync::MutexGuard<Option<u32>> as std::ops::DerefMut<Target = Option<u32>>>::deref_mut(from.lock().unwrap()).copied().unwrap(), handler_name.as_ptr() as i32, arg);
			};
		},
		None => quote! {},
	};
